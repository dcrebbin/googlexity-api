...
pub async fn googlexity(body: Json<SearchRequest>) -> Result<HttpResponse, Box<dyn Error>> {
    ...
    for query in split_search_queries {
        let search_items =
            google_search(actix_web::web::Json(GoogleSearchRequest { query: query })).await?;
...
pub async fn google_search(
    ...
    let google_search_response = match client
        .get(format!(
            ...
            search_api_key, search_engine_id, body.query
        ))
        .headers(headers)
        .send()
        .await
